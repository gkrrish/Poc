...
-- Sequence for DAILY_BANNER
DECLARE
    max_id NUMBER;
BEGIN
    SELECT NVL(MAX(location_id), 0) + 1 INTO max_id FROM DAILY_BANNER;

    EXECUTE IMMEDIATE 'CREATE SEQUENCE DAILY_BANNER_SEQ START WITH ' || max_id || ' INCREMENT BY 1 NOCACHE';
END;
